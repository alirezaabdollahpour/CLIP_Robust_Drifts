...
for dataset in [
    ...
    # I just change it
    my_dataset = "ImageNet"
    pretrained_checkpoint = f"{args.save}/{my_dataset}Val-epochs-10/linear_zeroshot.pt"
    print(f'pretrained_checkpoint is :{pretrained_checkpoint}')

    finetuned_checkpoint = (
        f"{args.save}/{my_dataset}Val-epochs-10/linear_finetuned.pt"
        if args.finetuning_mode == "linear"
        else f"{args.save}/{my_dataset}Val/finetuned.pt"
    )
    
    print(f'finetuned_chechpoint is :{finetuned_checkpoint}')
